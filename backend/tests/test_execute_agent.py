...
def mock_booking_agent():
    with patch("app.graph.orchestrator._get_booking_agent") as mock:
        agent = AsyncMock()
        agent.process.return_value = AgentResponse(response="Booking response", slots_filled={}, slots_missing=["origin"], is_complete=False)
...
def mock_faq_agent():
    with patch("app.graph.orchestrator._get_faq_agent") as mock:
        agent = AsyncMock()
        agent.process.return_value = AgentResponse(response="FAQ response", slots_filled={}, slots_missing=[], is_complete=True)
        mock.return_value = agent
        yield agent
...
async def test_execute_agent_low_confidence(mock_gsm):
    ...
    mock_gsm.get_stack.return_value = GoalStackState(session_id=uuid4(), goals=[], active_goal=None)
...
async def test_execute_agent_abandon_resumes_below(mock_gsm, mock_booking_agent):
    ...
    mock_gsm.get_stack.return_value = GoalStackState(session_id=session_id, goals=[resumed], active_goal=resumed)
...
@pytest.mark.asyncio
async def test_execute_agent_resume_invalid_target_regression(mock_gsm, mock_booking_agent):
    """
    ...
    mock_gsm.get_stack.return_value = GoalStackState(session_id=session_id, goals=[active], active_goal=active)
