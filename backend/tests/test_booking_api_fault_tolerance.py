...
async def test_api_retry_success(mock_goal):
    ...
    agent = BookingAgent()
    agent._simulate_failure_count = 2
    ...
    with patch.object(agent, '_generate_response', mock_generate):
        response = await agent.process(user_message="October 15th", goal=mock_goal)
...
async def test_api_retry_exhausted_graceful_degradation(mock_goal):
    ...
    agent = BookingAgent()
    agent._simulate_failure_count = 4  # More than max retries (3)
