...
class GoalStackManager:
    ...
    async def push_goal(self, session_id: UUID, intent_type: str) -> Goal:
        ...
        # 4. Insert the new active goal
        async with self._conn.execute(
            """
            INSERT INTO goal_stack
                (id, session_id, intent_type, status, slots_filled, slots_missing, stack_position)
            VALUES (?, ?, ?, 'active', ?, ?, ?)
            RETURNING *
            """,
            (
    ...
    async def resume_goal(self, session_id: UUID, goal_id: UUID) -> Goal:
        ...
        # 2. Move the target goal to the top of the stack
        async with self._conn.execute(
            """
    ...
    async def update_slots(
        ...
        async with self._conn.execute(
            ...
            RETURNING *
            """,
            (json.dumps(slots_filled), json.dumps(slots_missing), str(goal_id)),
        ) as cursor:
            row = await cursor.fetchone()
